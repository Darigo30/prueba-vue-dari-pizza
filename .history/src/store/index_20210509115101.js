  ...
  mutations: {
    ...
    comprar(state) {
      ...
      if (respuesta) {
        ...
        //   return obj2;
        // });
        state.carrito.forEach((car) => {
          if(state.ventas.length > 0){
            //console.log(state.ventas)
            const ventaanterior = state.ventas.find((obj) => obj.id === car.id);
             console.log(`venta anterior`, ventaanterior);
             if (ventaanterior) {
               ventaanterior.cantidadVendida += car.cantidadVendida;
               ventaanterior.precioSubtotal += car.precioSubtotal;
               console.log(`venta sumada`, ventaanterior);
             } else {
              const obj2 = {
                id: car.id,
                nombre: car.nombre,
                precioSubtotal: car.subtotal,
                cantidadVendida: car.cantidad,
              };
              state.ventas.push(obj2);
             }
