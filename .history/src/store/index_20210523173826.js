...
export default new Vuex.Store({
  ...
  actions: {
    ...
    async borrarPizzas({ commit }, payload) {
      ...
      console.log(payload);
      // Delete en Firestore
      const borrarFirebasedoc =  await db.collection("pizzas").where(pizza.id === pizza.payload);
        borrarFirebasedoc.get().then(function(querySnapshot) {
          querySnapshot.forEach(function(doc) {
            doc.ref.delete();
          })
          })
      //await db.collection("pizzas").doc("qg6tQAd5lqsRxK0jBO91").delete();
    }
  },
});
