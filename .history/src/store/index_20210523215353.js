...
export default new Vuex.Store({
  ...
  actions: {
    ...
    async crearNuevaPizza({ commit, state }, payload) {
      const pizza = payload;
      if (!pizza) return;

      const verificarId = state.productos.filter(pizza => pizza.id === payload );
      state.productos = verificarId;
      console.log("ya hay un Id con ese producto", verificarId);
      verificarId.innerHtml += `<div> ya existe el ID ${verificarId}</div>`;
