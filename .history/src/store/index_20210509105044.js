      ...
      if (respuesta) {
        ...
        const venta = state.carrito.map((obj) => {
          ...
          if(state.ventas.length > 0){
            const ventaregistro = state.ventas.find((obj) => obj.id === obj2.id);
            if (!ventaregistro) {
              state.venta.push(obj2);
            } else {
              ventaregistro.cantidad = obj2.cantidadVendida + ventaregistro.cantidad;
              ventaregistro.subtotal = ventaregistro.cantidad + obj2.precioSubtotal;
            }
