      ...
      if (respuesta) {
        ...
        const venta = state.carrito.map((obj) => {
          ...
          if(state.ventas.length > 0){
            ...
            const ventaanterior = state.ventas.find((obj) => obj.id === obj2.id);
             console.log(`venta anterior`, ventaanterior);
            // if (!ventaanterior) {
            //   state.ventas.push(obj2);
            // } else {
               ventaanterior.cantidadVendida = obj2.cantidadVendida + ventaanterior.cantidad;
               ventaanterior.precioSubtotal = ventaanterior.cantidad + obj2.precioSubtotal;
