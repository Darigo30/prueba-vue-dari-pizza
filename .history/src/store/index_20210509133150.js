        ...
        const venta = state.carrito.map((obj) => {
         ...
         const vendidos = state.ventas.map((vend) => {
            const obj3 = {
              id: vend.id,
              nombre: vend.nombre,
              precioSubtotal: vend.id === obj2.id ? vend.precioSubtotal + obj2.precioSubtotal : vend.precioSubtotal,
              cantidadVendida: vend.id === obj2.id ? vend.cantidadVendida + obj2.cantidadVendida : vend.cantidadVendida,
            };
            return obj3;
           // state.ventas.push(obj3);
              
          })
          if (state.ventas.length > 0) {
            console.log(vendidos)
            state.ventas = vendidos;
          } else state.ventas = obj2;
        });
