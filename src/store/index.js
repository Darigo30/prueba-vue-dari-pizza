...
export default new Vuex.Store({
  ...
  actions: {
    ...
    async crearNuevaPizza({ commit, state }, payload) {
      ...
      //Validar si existe una pizza con el mismo ID

      const verificarId = state.productos.filter(pizza =>  pizza.id === payload.id);
      if(verificarId.length !== 0){
        alert("ya existe un ID con ese producto", verificarId);
